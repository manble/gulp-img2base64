...
module.exports = function(opts) {
   ...
   return through.obj(function(file, enc, cb) {
      ...
      urlList.forEach((item) => {
         let imgPath = path.join(process.cwd(), opts.root, item);
         try{
            let base64Content = fs.readFileSync(imgPath, 'base64');
            contents = contents.replace(new RegExp(`([\\(\'"])${item}`), function($1, $2){
               return `${$2}data:${types[path.extname(imgPath)]};base64,${base64Content}`;
            });
         } catch(err) {
            cb(err, contents);
         }
